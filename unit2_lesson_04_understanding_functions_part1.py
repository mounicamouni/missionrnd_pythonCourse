...
def get_word_with_least_vowels(input):
    ...
    len=[]
    '''for i in inp:
       len.append(lenofvowel(i))

    return inp[len.index(min(len))]
    '''
    inp.sort(key=lenofvowel)
    return inp[0]
...
def get_min_max_words(input):
    ...
    input=list(input)
    #return input[input.index(min(input,key=len))],input[input.index(max(input,key=len))]
    input.sort(key=len)
