...
def test_for():
    ...
    for x in iter1:
        result.append(x)
    assert [] == result  #what happened here?---iter is pointing to end of list
...
def test_find_builtins_that_work_on_iterables():
    funcs = ['all','any','enumerate','filter','frozenset','list','map','max','min','next','set','sorted','sum','tuple','zip']
    assert 15 == len(funcs)
...
-types of iterators-list_iter,str_iter ...
-iter.__next__ ,stopIteration exception
