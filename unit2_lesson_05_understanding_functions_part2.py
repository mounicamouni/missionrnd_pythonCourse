...
def test_function_call_with_keyword_arguments():
    assert [10,30,3,4] == demo(10,30)
    assert [10,20,3,4] == demo(10, 20)
    assert [10,20,30,4] == demo(10, 20, 30)
    assert [1,20,3,4] == demo(1,second=20)
    assert [1,20,30,4] == demo(1,second=20, third=30)
    assert [10,2,30,4] == demo(first=10,second=2 , third=30)
    assert [10,2,30,4] == demo(10,second=2,third=30)
    # assert [__] == demo(second=20, 10)  # is this allowed? correct and uncomment--no,positional argument follows keyword argument
...
def test_function_unpacking():
    ...
    #
    result = demo_unpacking("jack", 1, 2,{3:"rw"}, k1="v1", k2="v2")
    print(result)
    assert ((1, 2, {3: 'rw'}), {'k1': 'v1', 'k2': 'v2'})==result
